...
/*------------------------------------------- 
create and show all product in UI start here
--------------------------------------------*/
const showProducts = (products) => {
  ...
  <div class="single-product  border border-info mb-3 rounded-3">
      <img class="product-image mt-3" src="${image}" alt="...">
      <div class="m-3">
          <div class="card-info">
              <h4 class="product-name">${product.title.slice(0, 50)}</h4>
              <p class="gray-text">Category: ${product.category}</p>
              <p class="gray-text">Rating: <i class="fa fa-star text-warning gray-text" aria-hidden="true"></i> <span class="fw-bold">${product.rating.rate}</span></p>
              <p class="gray-text">Review: <i class="text-danger fas fa-comment-alt"></i> <span class="fw-bold">${product.rating.count}</span></p>    
          </div>   
          <div>
              <h1 class="card-text mb-3 price">$ ${product.price}</h1>
                  <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-primary">add to cart</button>

                  <button id="details-btn" class="ms-2 btn btn-outline-primary">
                  Details
                  </button>       
          </div>
      </div>
...
/*------------------------------------------- 
create and show all product in UI end here
--------------------------------------------*/



/*--------------------------------------- 
  preparing input start here
----------------------------------------*/
let count = 0;

const prepareNum = (id) => {
  const numString = document.getElementById(id);
  const numText = parseFloat(numString.innerText);
  const num = numText;
  return num;
}
/*--------------------------------------- 
  preparing input end here
----------------------------------------*/
...
const addToCart = (id, price) => {
  ...
  const mainprice = prepareNum("price");
  const deliveryCharge = prepareNum("delivery-charge");
  const tax = prepareNum("total-tax");
  const totalFloat = mainprice + deliveryCharge + tax;
...
/*------------------------------------------ 
  main price update function start here
-------------------------------------------*/
